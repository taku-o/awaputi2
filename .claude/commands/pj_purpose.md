.kiro/specs/phase8-debug-app-infrastructure/requirements.md
.kiro/specs/phase8-debug-app-infrastructure/design.md
.kiro/specs/phase8-debug-app-infrastructure/tasks.md
に従って開発を進めていきます。

これらの資料と、docs/planning、docs/designのドキュメントを良く参照し、
tasks.mdに従って、順々に開発を進めていきます。

## 【絶対厳守】 タスク実行ルール
━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ 1回の応答で実行できるタスク数：1つだけ
━━━━━━━━━━━━━━━━━━━━━━━━━

実行手順：
1. 現在のタスクリストから最初の未完了項目を1つ選択
2. それがサブタスクなら、そのサブタスク1つだけを実行
3. それが独立タスクなら、そのタスク1つだけを実行
4. 【重要】実行が終わったら、次のタスクに進む前に必ず停止
5. ユーザーに進捗を報告
6. ユーザーの「続けて」等の指示があるまで待機
...
* 【絶対厳守】 ユーザーの許可なく、完了やcomplete、それに属するという文字を使ってはいけない、書き込んでもいけない。タスクリストに完了のチェックもつけてはいけない。部分完了や、一部完了なども不可である。実質完了という表現も不可です。タスクの完了判定はユーザーが行います。
...
* デバッグ時以外に、一時的な実装の置き換えをしてはいけません。簡略化も不可。
...
* ファイルを探して見つからなかった時に、ファイルやディレクトリを作成するまえに、まず現在ディレクトリを確認すること。
